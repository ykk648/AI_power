...
# Globals
male_model_path = 'pretrain_models/SMPL_series/SMPL/SMPL_m_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
female_model_path = 'pretrain_models/SMPL_series/SMPL/SMPL_f_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
...
def process_pose(current_frame, pose, trans, pelvis_position):
    ...
    bones[bone_name_from_index[0]].location = Vector(
...
def process_poses(
    ...
    json_files = sorted([str(i) for i in get_path_by_ext(input_path, ext_list=['.json'])])
    # print(json_files)
    ...
    for i in range(len(json_files)):
        ...
        poses.append(np.array(file_data[0]['poses']).reshape(-1, 3))
    print(len(poses))

    poses = np.array(poses)

    trans = np.zeros((len(json_files), 3))
...
if __name__ == '__main__':
    ...
    input_path = '/output/smpl/smpl'
    output_path = './test_easymocap.fbx'
    fps_source = 30
    fps_target = 30
