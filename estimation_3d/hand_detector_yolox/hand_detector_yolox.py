...
class HandDetect:
    ...
    def forward(self, image_p_):
        ...
        return np.array(results_after).astype(np.int)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        with MyFpsCounter() as mfc:
            hd_result = hd.forward(img)
        for bbox_ in hd_result:
            cv2.rectangle(img, (bbox_[0], bbox_[1]), (bbox_[2], bbox_[3]), (0, 255, 0), 4)
        CVImage(img).show(wait_time=1)
