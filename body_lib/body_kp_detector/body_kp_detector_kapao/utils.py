...
def post_process_batch(data, imgs, shapes, person_dets, kp_dets):
    ...
    for si, (pd, kpd) in enumerate(zip(person_dets, kp_dets)):
        ...
        if nd:
            ...
            bboxes = scale_coords(imgs[::-1], pd[:, :4], shape).round().cpu().numpy()
            poses = scale_coords(imgs[::-1], pd[:, -data['num_coords']:], shape).cpu().numpy()
            ...
            if data['use_kp_dets'] and nkp:
                ...
                if len(poses_mask):
                    kpd[:, :4] = scale_coords(imgs[::-1], kpd[:, :4], shape)
                    kpd = kpd[:, :6].cpu()
