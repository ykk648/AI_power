...
def resize_pad(img):
    ...
    Returns:
        ...
        pad: HW pixels of padding in the original image
    """
...
def postprocess(landmarks):
    ...
    for i in range(num):
        ...
        for j in range(33):
            ...
            visibility = xx[j * 5 + 3]
            presence = xx[j * 5 + 4]
            normalized_landmarks[i, j] = (x, y, z, expit(min(visibility, presence)))
...
def denormalize_detections(detections, scale, pad):
    ...
    """
    detections[:, 0] = detections[:, 0] * scale * 224 - pad[1]
    detections[:, 1] = detections[:, 1] * scale * 224 - pad[0]
    detections[:, 2] = detections[:, 2] * scale * 224 - pad[1]
    detections[:, 3] = detections[:, 3] * scale * 224 - pad[0]

    detections[:, 4::2] = detections[:, 4::2] * scale * 224 - pad[0]
    detections[:, 5::2] = detections[:, 5::2] * scale * 224 - pad[1]
    return detections
...
def detection2roi(detection, detection2roi_method='alignment'):
    ...
    if detection2roi_method == 'box':
        ...
        yc = (detection[:, 1] + detection[:, 3]) / 2
        xc = (detection[:, 0] + detection[:, 2]) / 2
        ...
        # x y reverse
        xc = detection[:, 4 + 2 * kp1 +1]
        yc = detection[:, 4 + 2 * kp1 ]
        x1 = detection[:, 4 + 2 * kp2+1]
        y1 = detection[:, 4 + 2 * kp2 ]
...
def extract_roi(frame, xc, yc, theta, scale):
    ...
    if imgs:
        ...
        imgs = np.zeros((1, res, res, 3))
        affines = np.zeros((1, 2, 3))
